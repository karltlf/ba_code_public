...
from src.features.get_x_y_tuple_list import * 
...
from src.models.optics.optimize_optics_parallelized import *
from src.models.optics.optimize_optics_parallelized_punish_non_edge_points import *
from src.features.get_first_and_last_x_y_coordinates import *


def plot_optics_optimized(df_path:str, max_eps_range:np.array, min_samples_range:np.array, metrics:list[str], cluster_methods:list[str], xis:np.array, **kwargs):
    ''' 
    Plots the optimized OPTICS clustering results for a given dataframe
    ...
    _, epsilon_optimized_bic, min_samples_optimized_bic, metric_optimized_bic, cluster_method_optimized_bic, xi_optimized_bic = optics_optimization_results['bic'].values()
    _, epsilon_optimized_aic, min_samples_optimized_aic, metric_optimized_aic, cluster_method_optimized_aic, xi_optimized_aic = optics_optimization_results['aic'].values()
    ...
    # 3.4 BIC-based optimized OPTICS model
    labels_bic, centers_bic = fit_optics(epsilon_optimized_bic, min_samples_optimized_bic, X, metric_optimized_bic, cluster_method_optimized_bic, xi_optimized_bic, **kwargs)

    # 3.5 AIC-based optimized OPTICS model
    labels_aic, centers_aic = fit_optics(epsilon_optimized_aic, min_samples_optimized_aic, X, metric_optimized_aic, cluster_method_optimized_aic, xi_optimized_aic, **kwargs)
...
def fit_optics(epsilon, min_samples, X, metric, cluster_method, xi, **kwargs):
        ...
        if metric_params is None:
            if cluster_method == 'xi':
                optics = OPTICS(max_eps=epsilon, min_samples=min_samples, metric=metric, cluster_method=cluster_method, xi=xi)
            else:
                optics = OPTICS(max_eps=epsilon, min_samples=min_samples, metric=metric, cluster_method=cluster_method)
        else:
             optics = OPTICS(max_eps=epsilon, min_samples=min_samples, metric=metric, cluster_method=cluster_method, metric_params=metric_params)
        optics.fit(X)
